...
class TripletFoodDataset_1(Dataset):
    ...
    def __init__(self, root, split='train', transforms=None, triplet=True, torchvision=True):
        ...
        self.samples = np.loadtxt(self.split,dtype=str ,delimiter=' ')
    ...
    def __getitem__(self, item):
        if not self.triplet:
            ...
            image_paths = [os.path.join(self.food_dir, i+'.jpg') for i in self.samples[item]]
...
class TripletFoodDataset(Dataset):
    ...
    def __init__(self, root, split='train', transforms=None, triplet=True, torchvision=True):
        ...
        self.food_dir = os.path.join(root,'food')
        self.imgs = os.listdir(self.food_dir)
        self.split = os.path.join('./', f'{split}_triplets.txt')
        self.samples = np.loadtxt(self.split,dtype=str ,delimiter=' ')
        self.triplet = triplet
        self.transforms = transforms
        self.torchvision = torchvision
...
class TripletLoss(nn.Module):
    ...
    def forward(self, anchor, positive, negative):
        distance_positive = (anchor - positive).pow(2).mean(1)
        distance_negative = (anchor - negative).pow(2).mean(1)
