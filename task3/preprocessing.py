...
def main():
    ...
    train_data = pd.read_csv(os.path.join(data_path, 'train.csv')).values
    train_features = train_data[:, 0]  # (112000,)
    train_labels = train_data[:, 1]  # {0: 107787, 1: 4213}
    train_labels = train_labels.astype('int')
    test_data = pd.read_csv(os.path.join(data_path, 'test.csv')).values  # (48000, 1)
    train_features_res, train_labels_res = train_features, train_labels
    # sampler = RandomOverSampler()
    # train_features_res, train_labels_res = sampler.fit_resample(train_features.reshape(-1, 1),
    ...
    saved_Train_label_path = os.path.join(data_path, "oversampled_labels.npy")
    if os.path.exists(saved_Train_data_path) and os.path.exists(saved_Test_data_path):
        ...
        np.save(saved_Train_label_path, train_labels_res)
    ...
    # --------------------------Refitting--------------------------------
    print("Refitting ... model is stored in ", os.path.join(model_path, "MLPclassifier.pkl"))
    clf.best_estimator_.fit(train_features_res_enc, train_labels_res)
    joblib.dump(clf.best_estimator_, os.path.join(model_path, "MLPclassifier.pkl")) """

    # --------------------------Predicting--------------------------------
    if os.path.exists(os.path.join(model_path, "MLPclassifier.pkl")):
        clf_model = joblib.load(os.path.join(model_path, "MLPclassifier.pkl"))
