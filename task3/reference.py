...
def train():
    # --------------------------preprocessing--------------------------------
    train_data, train_label = load_data(os.path.join(data_path,'train.csv'))
    test_data = load_data(os.path.join(data_path,'test.csv'), False)
    ...
    pd.DataFrame(test_pred).to_csv('proba13.csv', index=False, header=None)
    test_pred = np.where(test_pred > 0.5, 1, 0)
    pd.DataFrame(test_pred).to_csv('submission13.csv', index=False, header=None)
